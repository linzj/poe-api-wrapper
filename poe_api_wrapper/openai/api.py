...
from typing import (
    ...
    Any,
    Union,
)  # Added Any for message content & Union
...
class PromptCacheManager:
    ...
    def __init__(
        self,
        cache_store: Dict[str, Dict[str, Union[str, "AsyncPoeApi", bool]]],
        ...
        if self.logging_enabled:
            print(
                f"[CACHE_MANAGER_INIT] PromptCacheManager initialized. Current cache size: {len(self.cache_store)}"
            )
    ...
    def _generate_cache_key(self, content: Any) -> Optional[str]:
        ...
        if isinstance(content, str):
            if content:  # Ensure string is not empty
                ...
                self._log(
                    f"[CACHE_MANAGER_KEYGEN_SKIP] Content is an empty string. Skipping key generation."
                )
                return None
        elif isinstance(
            content, (list, dict)
        ):  # Handle list or dict content types (e.g. for multimodal)
            if not content:  # Ensure list/dict is not empty
                self._log(
                    f"[CACHE_MANAGER_KEYGEN_SKIP] Content is an empty list/dict. Skipping key generation."
                )
                return None
            try:
                ...
                self._log(
                    f"[CACHE_MANAGER_KEYGEN_SERIALIZE_FAIL] Failed to serialize {content_type_str} content for key: {e}. Content preview: '{str(content)[:150]}...'"
                )
                return None
            ...
            self._log(
                f"[CACHE_MANAGER_KEYGEN_WARN] Content type {content_type_str} is not str/list/dict. Attempting str() conversion for key. Preview: '{str(content)[:100]}...'"
            )
        ...
        if key_string:  # Check if key_string was successfully set and is not empty
            try:
                ...
                self._log(
                    f"[CACHE_MANAGER_KEYGEN] Generated cache key: {key} from content (type: {content_type_str}, first 100 chars of serialized_form): '{key_string[:100]}...'"
                )
                return key
            except Exception as e:
                self._log(
                    f"[CACHE_MANAGER_KEYGEN_HASH_FAIL] Failed to hash string_repr for key: {e}. String preview: '{key_string[:150]}...'"
                )
                return None

        self._log(
            f"[CACHE_MANAGER_KEYGEN_FAIL] Failed to generate key, content not suitable or empty. Original content type: {content_type_str}, preview: '{str(content)[:150]}...'"
        )
        return None

    def check_cache(self, messages: List[Dict[str, Any]]) -> Tuple[
        Optional["AsyncPoeApi"],
        Optional[bool],
        Optional[str],
        List[Dict[str, Any]],
        bool,
        ...
        if len(messages) >= 2:
            ...
            if cache_key_for_storage and cache_key_for_storage in self.cache_store:
                ...
                if (
                    ret_client is not None
                    and ret_subscription is not None
                    and ret_chat_id is not None
                ):
                    ...
                    self._log(
                        f"[CACHE_MANAGER_HIT_FULL] FULL CACHE HIT: Using cached client, subscription, and chatId '{chat_id_to_use}' for key {cache_key_for_storage}."
                    )
                else:  # Partial hit (e.g. key exists but data incomplete), treat as client/sub miss
                    self._log(
                        f"[CACHE_MANAGER_HIT_PARTIAL] PARTIAL CACHE HIT: Key {cache_key_for_storage} found, but client/sub/chat_id incomplete. Will proceed as client miss."
                    )
                ...
                if processed_messages:
                    ...
                    self._log(
                        f"[CACHE_MANAGER_MSG_TRIM] Messages for key {cache_key_for_storage} trimmed from {original_message_count} to 1."
                    )

            elif cache_key_for_storage:
                self._log(
                    f"[CACHE_MANAGER_MISS] CACHE MISS (initial check): Key {cache_key_for_storage} not found in cache store."
                )
        else:
            self._log(
                f"[CACHE_MANAGER_SKIP] Skipping cache check: Not enough messages for key generation (count: {len(messages)})."
            )
    ...
    def store_in_cache(
        ...
        if not cache_key:
            self._log(
                f"[CACHE_MANAGER_STORE_FAIL:{source.upper()}] No cache key provided for bot '{bot_name}'. Cannot store."
            )
            return
        if not chat_id:
            self._log(
                f"[CACHE_MANAGER_STORE_FAIL:{source.upper()}] No (or empty) chat_id provided for bot '{bot_name}' with key '{cache_key}'. Cannot store."
            )
            return
        if client_instance is None:  # Also check client_instance
            self._log(
                f"[CACHE_MANAGER_STORE_FAIL:{source.upper()}] No client_instance provided for bot '{bot_name}' with key '{cache_key}'. Cannot store."
            )
            return
        ...
        self._log(
            f"[CACHE_MANAGER_STORE:{source.upper()}] Stored chatId '{chat_id}', client instance, and subscription ({subscription_status}) for key '{cache_key}' (bot: {bot_name}). Cache size: {len(self.cache_store)}"
        )
...
POE_API_CACHE_LOGGING_ENABLED_STR = os.environ.get(
    "POE_API_CACHE_LOGGING_ENABLED", "false"
)
CACHE_LOGGING_ENABLED = POE_API_CACHE_LOGGING_ENABLED_STR.lower() == "true"
if CACHE_LOGGING_ENABLED:
    print(
        f"[CACHE_INIT] Prompt caching logging is ENABLED (POE_API_CACHE_LOGGING_ENABLED='{POE_API_CACHE_LOGGING_ENABLED_STR}')"
    )
else:
    print(
        f"[CACHE_INIT] Prompt caching logging is DISABLED (POE_API_CACHE_LOGGING_ENABLED='{POE_API_CACHE_LOGGING_ENABLED_STR}')"
    )
...
async def chat_completions(
    ...
    messages_data, model, streaming, max_tokens, stream_options, tools, tool_choice = (
        data.messages,
        data.model,
        data.stream,
        data.max_tokens,
    ...
    if not await helpers.__validate_messages_format(
        messages
    ):  # `messages` might be trimmed here
        raise HTTPException(
            detail={
                "error": {
    ...
    if (
        cached_client and cached_subscription is not None and chat_was_reused
    ):  # Re-evaluate condition based on new `check_cache`
        ...
        if CACHE_LOGGING_ENABLED:  # CACHE_LOGGING_ENABLED assumed to be global
            app.state.prompt_cache_manager._log(
                f"[CHAT_COMPLETIONS] Using cached client and subscription for key {cache_key_for_storage}."
            )
    else:
        if CACHE_LOGGING_ENABLED:
            ...
            app.state.prompt_cache_manager._log(
                f"[CHAT_COMPLETIONS] Rotating token: {log_reason}."
            )
        client_to_use, subscription_to_use = await rotate_token(app.state.tokens)

    if (
        client_to_use is None
    ):  # Should not happen if rotate_token works, but as a safeguard
    ...
    text_messages, image_urls = await helpers.__split_content(
        messages
    )  # Use potentially trimmed `messages`
    ...
    response_dict = await message_handler(
        baseModel, text_messages, tokensLimit
    )  # Pass text_messages
    prompt_tokens = await helpers.__tokenize(
        "".join([str(message) for message in response_dict["message"]])
    )
...
async def generate_chunks(
    ...
    client_for_cache_storage: Optional[
        AsyncPoeApi
    ] = None,  # The client instance that generated the response
    subscription_for_cache_storage: Optional[
        bool
    ] = None,  # The subscription status of that client
    ...
    first_chunk_processed_for_cache_store = (
        False  # To ensure store_in_cache is called only once for this request
    )
    try:
        ...
        if not raw_tool_calls:
            ...
            async for chunk in client.send_message(
                ...
                if (
                    not chat_was_reused
                    and cache_key_to_store
                    and bot_name
                    and not first_chunk_processed_for_cache_store
                ):
                    retrieved_chat_id = (
                        client.last_chat_id
                    )  # Chat ID generated by the send_message call
                    if (
                        retrieved_chat_id
                        and client_for_cache_storage is not None
                        and subscription_for_cache_storage is not None
                    ):
                        ...
                        app.state.prompt_cache_manager._log(
                            f"[GENERATE_CHUNKS_CACHE_STORE_SKIP:STREAM] Could not store. Missing retrieved_chat_id ({retrieved_chat_id is not None}), client_for_cache_storage ({client_for_cache_storage is not None}), or subscription_for_cache_storage ({subscription_for_cache_storage is not None})."
                        )
            ...
            end_completion_data = await create_completion_data(
                ...
                prompt_tokens=prompt_tokens,
                completion_tokens=(
                    chunk_token if "chunk_token" in locals() else 0
                ),  # Handle case where loop might not run
            )
...
async def streaming_response(
    ...
    return StreamingResponse(
        content=generate_chunks(
            ...
            chat_was_reused,
            cache_key_to_store,
            client_for_cache_storage,
            subscription_for_cache_storage,  # Pass down
        ),
        status_code=200,
        headers={
    ...
    if not raw_tool_calls:
        try:
            ...
            async for chunk in client.send_message(
                ...
                if (
                    max_tokens
                    and await helpers.__tokenize(final_chunk_text) >= max_tokens
                ):
                    await client.cancel_message(chunk)
                    finish_reason = "length"
                    break
        ...
        if not chat_was_reused and cache_key_to_store and bot_name:
            ...
            if (
                ...
                app.state.prompt_cache_manager._log(
                    f"[NON_STREAMING_CACHE_STORE_SKIP:NON_STREAM] Could not store. Missing retrieved_chat_id ({retrieved_chat_id is not None}), client_for_cache_storage ({client_for_cache_storage is not None}), or subscription_for_cache_storage ({subscription_for_cache_storage is not None})."
                )
        # --- END STORE CLIENT & CHAT_ID ON CACHE MISS ---
        ...
        chunk_text_for_tokens = (
            ""  # Represents the final message content, empty for tool_calls
        )
        finish_reason = "tool_calls"
        ...
        choices=[
            ChatCompletionResponseChoice(
                ...
                message=MessageResponse(
                    role="assistant",
                    content=None if raw_tool_calls else chunk_text_for_tokens,
                    tool_calls=(
                        [
